...
export default function TestimonialsSection({ locale }: { locale: 'fr' | 'en' }) {
  ...
  return (
    <section className="bg-white pt-12 pb-10">
      <div
        ref={headerRef}
        className={`transition-all duration-700 ease-out ${headerVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}
      >
      ...
      <style jsx>{`
        ...
        @media (prefers-reduced-motion: reduce) {
          .kb-testimonial-arrow { transition: none !important; }
        }
      `}</style>
    </section>
  )
